...
{
		if (objButton.innerHTML=="Добавить") {
			objButton.innerHTML = "Убрать";
			arr.push(objButton.value)
		}
		else {
			objButton.innerHTML = "Добавить";
			arr = arr.filter(val => val !== objButton.value);
		}
		what_coffee_is()
}

/**
 * Determines the appropriate type of coffee
 * @returns nothing
 */
function what_coffee_is()
{
	for (let j = 0; j < Object.keys(types).length; j++)
	{
		if (Object.keys(arr).length === types[j].ingredients.length)
		{
			b = true
			var mass = []
			for (let u = 0; u < types[j].ingredients.length; u++)
			{
				mass.push(types[j].ingredients[u])
			}
			for (let i = 0; i < Object.keys(arr).length; i++) 
			{
				if (typeof(mass.find(element => element == arr[i])) === 'undefined')
				{
					b = false
				}
			}
			if (b === true)
			{
				document.getElementById('coffimg').src=types[j].image;
				document.getElementById('cardheader').innerHTML=types[j].title;
				document.getElementById('cardfooter').innerHTML=types[j].description;
				return 
			}
		}
	}
	document.getElementById('coffimg').src="https://crosti.ru/patterns/00/03/4d/46_picture_c045620d.jpg";
	document.getElementById('cardheader').innerHTML="Нет такого кофе";
	document.getElementById('cardfooter').innerHTML="Попробуйте другие ингридиенты";
	return 
}
